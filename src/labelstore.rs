...
use crate::store::DigestString;
pub trait LabelstoreInterface {
    fn get(&self, digest: DigestString) -> Option<HashMap<String, String>>;
    fn set(&mut self, digest: DigestString, maps: HashMap<String, String>) -> Result<(), Box<dyn Error>>;
    fn update(
        &mut self,
        digest: DigestString,
...
impl Labelstore {
    pub fn new(rt: String) -> Self {
        ...
        let _ = fs::create_dir_all(ingest_path.clone());
        if let Err(_) = fs::set_permissions(ingest_path, Permissions::from_mode(0o777)) {
